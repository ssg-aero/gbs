...
TEST(tests_vtk_render, BSC) {
  ...
  std::vector<std::array<double, 4>> poles2 = {
      {r1, r2, 0., 1.}, {0.5 * (r1 + c), r2, 0., 1.}, {c, r2, 0., 1.}};
  ...
  std::transform(crv3.poles().begin(), crv3.poles().end(),
                 poles_non_rational.begin(),
                 [](const auto &p_) { return gbs::weight_projection(p_); });
...
TEST(tests_vtk_render, BSC_option) {
  ...
  gbs::plot(gbs::crv_dsp<double, 3, false>{
      .c = &c1,
      .col_crv = {1., 0., 0.},
      .poles_on = true,
      .col_poles = {0., 1., 0.},
...
TEST(tests_vtk_render, dev) {
  ...
  std::vector<std::array<double, 4>> poles4 = {
      {0., 0., 0., 1.},
      {0.33 * t1 * c * cos(b1), 0.33 * t1 * c * sin(b1), 0., 1.},
      {c * cos(g) - 0.33 * t2 * c * cos(b2),
       c * sin(g) - 0.33 * t2 * c * sin(b2), 0., 1.},
      {c * cos(g), c * sin(g), 0., 1.}};
  ...
  std::for_each(std::next(k4.begin(), 3), std::next(k4.end(), -3),
                [&](const auto k_) { crv3.insertKnot(k_); });
  std::for_each(std::next(k3.begin(), 3), std::next(k3.end(), -3),
                [&](const auto k_) { crv4.insertKnot(k_); });

  std::vector<double> u(crv3.poles().size());

  u[0] = 0.;
  u[1] = 0.;
  std::transform(std::next(crv3.poles().begin(), 2), crv3.poles().end(),
                 std::next(u.begin(), 2), [&](const auto &pt_) {
                   return gbs::extrema_curve_point(
                       crv3, gbs::weight_projection(pt_), 1e-6)[0];
                 });

  std::vector<std::array<double, 4>> poles5(crv3.poles().size());
  // std::transform(crv3.poles().begin(),crv3.poles().end(),
  // crv4.poles().begin(),poles5.begin(),
  // [&](const auto &thick,const auto &cl){
  //     auto u_cl =
  //     gbs::extrema_curve_point(crv4,gbs::weight_projection(cl),1e-6).u; auto
  //     t_cl = crv4.value(u_cl,1); std::array<double,3> n_cl =
  //     {-t_cl[1],t_cl[0],0};
  ...
  //     std::array<double,4>
  //     p{cl[0]+n_cl[0]*ep,cl[1]+n_cl[1]*ep,cl[2]+n_cl[2]*ep,thick[3]}; return
  //     p;
  //     });
...
TEST(tests_vtk_render, surf_points) {
  ...
  auto blue_col = gbs::array_from_col("Blue");
  gbs::plot(gbs::make_actor(points, 10., true, blue_col), gbs::make_actor(srf));
}
...
public:
  static InteractorStyleEditCurve *
  New() // Can't make vtkStandardNewMacro works with templates
  {
  ...
  // void setCurve(gbs::BSCurveGeneral<T,dim,rational> *p_crv,
  // vtkSmartPointer<vtkAssembly> p_actor)
  // {
  //     p_crv_ = p_crv;
  //     p_actor_ = p_actor;
  // }
