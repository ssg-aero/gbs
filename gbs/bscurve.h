 ...
 */
    ...
    public:
        ...
        virtual auto value(T u, size_t d = 0) const -> std::array<T, dim> override
        {
            if (d == 0)
            {
                return weight_projection(gbs::EvalValueRecursive(u, knotsFlats(), poles(), degree(), d, true));
            }
            else
            {
                // auto wu = value(u).back();
                auto wu = gbs::EvalValueRecursive(u, knotsFlats(), poles(), degree(), 0, false).back();
                // auto Ckw= value(u,d);
                auto Ckw = gbs::EvalValueRecursive(u, knotsFlats(), poles(), degree(), d, false);
                Ckw.back() = 1.;
                ...
                for (int i = 1; i <= d; i++)
                {
                    ...
                    auto wi = gbs::EvalValueRecursive(u, knotsFlats(), poles(), degree(), i, false).back();
                    auto C = value(u, d - i);
    ...
    public:
        ...
        virtual auto value(T u, size_t d = 0) const -> std::array<T, dim> override
        {
            return gbs::EvalValueRecursive(u, knotsFlats(), poles(), degree(), d);
        }
    };
